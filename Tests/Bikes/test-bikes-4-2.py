...
def test_mariadb_query():
    ...
    try:
        ...
        cursor = conn.cursor()
        query = "SELECT * FROM Stations WHERE station_id IN (SELECT DISTINCT end_station_id FROM TripUsers);"  
...
def test_mongodb_query():
    ...
    try:
        ...
        db = client['Bikes']
        trip_users_collection = db['TripUsers']
        stations_collection = db['Stations']
        
        end_station_ids = trip_users_collection.distinct('end_station_id')
        
        pipeline = [
            {'$match': {'station_id': {'$in': end_station_ids}}},
        ...
        cursor = stations_collection.aggregate(pipeline)
