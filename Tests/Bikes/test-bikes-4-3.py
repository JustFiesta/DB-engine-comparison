...
def test_mariadb_query():
    ...
    try:
        ...
        cursor = conn.cursor()
        query = "SELECT * FROM TripUsers WHERE birth_year < 1980 AND tripduration > (SELECT AVG(tripduration) FROM TripUsers WHERE birth_year < 1980);"  
...
def test_mongodb_query():
    ...
    try:
        ...
        avg_duration_young = collection.aggregate([
            {'$match': {'birth_year': {'$lt': 1980}}},
            {'$group': {'_id': None, 'avg_tripduration': {'$avg': '$tripduration'}}}
        ]).next()['avg_tripduration']

        pipeline = [
            {'$match': {'birth_year': {'$lt': 1980}, 'tripduration': {'$gt': avg_duration_young}}},
            {'$project': {'_id': 0, 'trip_id': 1, 'tripduration': 1, 'birth_year': 1}}
        ]
